...
public class IntCellUnaryEqConstraint extends UnaryConstraint {
	...
	public IntCellUnaryEqConstraint(ACQ_CellVariable cell1, int constant) {
		...
		if (v1.getType() != CellType.INT && v1.getType() != CellType.INT8 && v1.getType() != CellType.UINT && v1.getType() != CellType.UINT8
				&& v1.getType() != CellType.UINT16 && v1.getType() != CellType.BOOL) {
	...
	public String getBwConstraint() {
		...
		String c = String.format("(= %s %s)", val1_value, cst);
		if (!negated)
			return c;
		else
	...
	public String toSmtlib() {
		...
		case INT:
			expr = "(eq " + val1 + " " + String.format("#x%08x", this.constant) + ")";
			break;
		case INT8:
			expr = "(eq " + val1 + " " + String.format("#x%02x", this.constant) + ")";
			break;
		case UINT:
			expr = "(eq " + val1 + " " + String.format("#x%08x", this.constant) + ")";
			break;
		case UINT8:
			...
			break;
		case UINT16:
			expr = "(eq " + val1 + " " + String.format("#x%04x", this.constant) + ")";
			break;
		case BOOL:
			expr = "(eq " + val1 + " " + String.format("#x%02x", this.constant) + ")";
			break;
