...
public abstract class ACQ_Constraint implements ACQ_IConstraint {
	...
	public Bitvec[] getProjection(ACQ_Query query) {
		int index = 0;
		Bitvec[] values = new Bitvec[variables.length];
	 ...
	 */
	@Override
	public final boolean checker(Bitvec[] values) {
		return check(values);
	}
	/**
	 * Checks if the constraint is violated for a given set of values
	...
	public String getBwConstraint() {
		assert false : "Bitwuzla ver of the constraint is not implemented ";
		return null;
	}

	public String toBinsecConstraint(ACQ_Query q, Map<String, String> regid) {
		assert false : "not implemented tobinsecconstraint";
		return null;
	}

	public boolean check(Bitvec... value) {
		assert false : "check is not implemented for this constraint";
		return false;
	}
	...
	@Override
	public ConstraintSet toConstraintSet(ConstraintFactory cf) {
		return cf.createSet(this);
	}

	@Override
	public boolean isSubsumed(ACQ_IConstraint c, ConstraintFactory cf) {
		ConstraintSet c1 = this.toConstraintSet(cf);
		ConstraintSet c2 = c.toConstraintSet(cf);
		return c1.isSubsumed(c2);
	}
}
