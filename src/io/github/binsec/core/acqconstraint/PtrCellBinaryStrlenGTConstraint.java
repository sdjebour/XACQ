...
public class PtrCellBinaryStrlenGTConstraint extends BinaryConstraint {
	boolean negated = false;
	ACQ_CellVariable cell1;
	ACQ_CellVariable cell2;
	ACQ_Variable[] vars; // length = 2

	public PtrCellBinaryStrlenGTConstraint(ACQ_CellVariable cell1, ACQ_CellVariable cell2) {
		super("StrlenGT", cell1.getSize().id, cell2.getSize().id); // TODO : WARNING I don't know if it is good
		...
		if (v1.getType() != CellType.STR || v2.getType() != CellType.STR) {
			System.err.println("Constraint badly typed");
			System.exit(1);
		} else {
			this.vars = new ACQ_Variable[] { cell1.getSize(), cell2.getSize() };
		}
	}

	public PtrCellBinaryStrlenGTConstraint(ACQ_CellVariable cell1, ACQ_CellVariable cell2, boolean negated) {
		this(cell1, cell2);
		if (negated) {
			this.setName("StrlenLE");
	...
	public String getBwConstraint() {
		String val1_value = "v" + this.cell1.getSize().id;
		String val2_value = "v" + this.cell2.getSize().id;
		String c = String.format("(bvugt %s %s)", val1_value, val2_value);
		if (!negated)
			return c;
		else
	...
	public String toBinsecConstraint(ACQ_Query q, Map<String, String> regid) {
		...
		assert false : "binary strleneq to fix";
	...
	public ACQ_IConstraint getNegation() {
		return new PtrCellBinaryStrlenGTConstraint(this.cell1, this.cell2, !this.negated);
	}
	...
	public String toSmtlib() {
		String p = "(strlengt v" + this.cell1.cellid + " v" + this.cell2.cellid + ")";
		if (!negated)
			return p;
		else
			return "(not " + p + ")";
	...
	protected boolean check(Bitvec value1, Bitvec value2) { // Better if prams could be cells
		if (negated) {
			return value1.ucompareTo(value2) <= 0;
		} else {
			return value1.ucompareTo(value2) > 0;
		}
	}
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		PtrCellBinaryStrlenGTConstraint other = (PtrCellBinaryStrlenGTConstraint) obj;
