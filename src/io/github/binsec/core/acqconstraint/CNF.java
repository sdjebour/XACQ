...
public class CNF implements Iterable<Clause> {
	...
	public ACQ_Query getInconsistency() {
		assert this.isMonomial();
		for (Clause cl : clauses) {
			if (cl.getOriginQuery() == null)
				continue; // May append in IrredundantQuery
			Unit unit = cl.get(0);
			if (!unit.isNeg()) {
				for (Clause cl2 : clauses) {
					Unit unit2 = cl2.get(0);
					if (unit2.isNeg() && unit.getConstraint().equals(unit2.getConstraint())) {
