...
public class BinaryArithmetic extends BinaryConstraint {
	...
	public String getBwConstraint() {
		//System.out.println("WARNING: binary op is unsigned by default");
		String var1 = "v" + variables[0]; 
		String var2 = "v" + variables[1];
		...
		case GT:
			return "(bvugt " + var1 + " " + var2 + ")";
		case GE:
			return "(bvuge " + var1 + " " + var2 + ")";
		case LT:
			return "(bvult " + var1 + " " + var2 + ")";
		case LE:
			return "(bvule " + var1 + " " + var2 + ")";
		default:
	 ...
	 */
	@Override
	protected boolean check(Bitvec value1, Bitvec value2) {
		//System.out.println("WARNING: Binary arithmetic op are usinged by default");
		Bitvec val1;
		Bitvec val2;
		Operator op;
...
//			return !check_InDiag2(value1, value2);
		...
		if(op2 == Operator.NONE) {
			...
			if (isArithmOperation(op1)) {
				...
				if (op1 == Operator.Dist) {
					val1 = (value1.subtract(value2)).abs();
				}else 
					if (op1 == Operator.PL) {
						val1 = value1.add(value2);
					} else {
						val1 = value1.subtract(value2);
					}
				val2 = new Bitvec(BigInteger.valueOf(cste), val1.size);
				op = op2;
			} else {
				Bitvec bvcst = new Bitvec(BigInteger.valueOf(cste), 32);
				if (op2 == Operator.PL) {
					val2 = value2.add(bvcst);
				} else {
					val2 = value2.subtract(bvcst);
				}
				val1 = value1;
				op = op1;
			}
