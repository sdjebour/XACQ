...
public class IntCellUnaryGTConstraint extends UnaryConstraint {
	...
	public String getBwConstraint() {
		...
		if (cell1.getType() == CellType.INT || cell1.getType() == CellType.INT8) {
			c = String.format("(bvsgt %s %s)", val1_value, cst);
		} else {
			c = String.format("(bvugt %s %s)", val1_value, cst);
		}
		if (!negated)
			return c;
		else
	...
	public String toSmtlib() {
		...
		case INT:
			expr = "(sgt " + val1 + " " + String.format("#x%08x", this.constant) + ")";
			break;
		case INT8:
			expr = "(sgt " + val1 + " " + String.format("#x%02x", this.constant) + ")";
			break;
		case UINT:
			expr = "(ugt " + val1 + " " + String.format("#x%08x", this.constant) + ")";
			break;
		case UINT8:
			expr = "(ugt " + val1 + " " + String.format("#x%02x", this.constant) + ")";
			break;
		case UINT16:
			expr = "(ugt " + val1 + " " + String.format("#x%04x", this.constant) + ")";
			break;
		default:
