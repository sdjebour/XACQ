...
public class PtrCellEndSameAddr extends ScalarConstraint {
	...
	public PtrCellEndSameAddr(ACQ_CellVariable cell1, ACQ_CellVariable cell2) {
	...
	public PtrCellEndSameAddr(ACQ_CellVariable cell1, ACQ_CellVariable cell2, boolean negated) {
		this(cell1, cell2);
		if (negated) {
			this.setName("NotEndSameAddr");
	...
	public String getBwConstraint() {
		...
		String c = " (= (bvadd " + val1_value + " ((_ zero_extend " + diff + ") " + val1_size + ")) (bvadd "
				+ val2_value + " ((_ zero_extend " + diff + ") " + val2_size + ")))";
		if (!negated)
			return c;
		else
	...
	public String toBinsecConstraint(ACQ_Query q, Map<String, String> regid) {
		...
		String strlen1 = "#x" + String.format("%0" + n + "x", q.getValue(cell1.getSize().id).unsigned().intValue());

		String strlen2 = "#x" + String.format("%0" + n + "x", q.getValue(cell2.getSize().id).unsigned().intValue());
		...
		// String c = String.format("(and (= %s %s) (not (= %s %s)) (not (= %s %s)))",
		// val1_value, val2_value, val1_value,
		// hexcst, val2_value, hexcst);
		String c = String.format("(= (bvadd %s %s) (bvadd %s %s))", val1_value, strlen1, val2_value, strlen2);
		if (!negated)
			return c;
		else
			return String.format("(not %s)", c);
	...
	public PtrCellEndSameAddr(ACQ_CellVariable cell1, ACQ_CellVariable cell2, int weight) {
		this(cell1, cell2);
		this.weight = weight;
	}

	public PtrCellEndSameAddr(ACQ_CellVariable cell1, ACQ_CellVariable cell2, boolean negated, int weight) {
	...
	public ACQ_IConstraint getNegation() {
		return new PtrCellEndSameAddr(this.cell1, this.cell2, !this.negated);
	}

	public ACQ_IConstraint getNegation(int weight) {
		return new PtrCellEndSameAddr(this.cell1, this.cell2, !this.negated, weight);
	}
	...
	public String toSmtlib() {
		String o = "(EndSameAddr v" + this.cell1.cellid + " v" + this.cell2.cellid + ")";
		if (!negated)
			return o;
		else
			return "(not " + o + ")";
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		PtrCellEndSameAddr other = (PtrCellEndSameAddr) obj;
	...
	protected boolean check(Bitvec[] value, int[] coeff) {
		...
		if (!negated) {
			return (v0.add(v0size).equals(v1.add(v1size)));
