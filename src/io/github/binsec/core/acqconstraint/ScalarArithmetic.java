...
public class ScalarArithmetic extends ScalarConstraint {
	 ...
	 * @example X + Y -Z < 0
	 */
	public ScalarArithmetic(String name, int[] var, int[] coeif, Operator op, int cste, int size) {
	...
	public ScalarArithmetic(String name, int[] var, int[] coeif, Operator op, int cste, String negation, int size) {
		super(name, var, coeif, cste);
		this.op1 = op;
		this.size=size;
		this.cste = cste;
	...
	public String getBwConstraint() {
		assert false : "need fixing";
		String res = "(bvadd";
		for (int i = 0; i < variables.length; i++) {
			int var = variables[i];
			String coef = "#b" + String.format("%0"+size*4+"b", coeff[i]);
			res = res + " (bvmul " + coef + " v" + var + ")";
		}
		res = res + ")";
		String hexcste = "#b" + String.format("%0"+size*4+"b", cste);
		return "("+ opToSmtlib(op1) + " " + res + " " + hexcste + ")"; 
	...
	public ScalarArithmetic getNegation() {
		...
		return new ScalarArithmetic("not_" + getName(), this.vars, this.coeff, Operator.getOpposite(op1), cste, size);
	...
	protected boolean check(Bitvec[] vars, int[] coeff) {
		Bitvec val1 = Bitvec.zero(vars[0].size);

		for (int i = 0; i < vars.length; i++) {
			val1 = val1.add(vars[i].multiply(new Bitvec(BigInteger.valueOf(coeff[i]), val1.size)));
		}
	...
	public String toSmtlib() {
		...
		for (int i = 0; i < variables.length; i++) {
			int var = variables[i];
			int coef = coeff[i];
			String coefhexa = String.format("%04x", coef);
			//System.out.println("le coef en hexa : "+coefhexa+" sa taille : "+coefhexa.length());
			String coefhexafixed=coefhexa.substring(coefhexa.length()-4);
			
			//System.out.println("THIS IS THE HEXA : "+ coefhexafixed);
			//String scoef = String.format("#x%04x", coef);
			res = res + " (mul " + "#x"+coefhexafixed + " " + varToSmtlib(var) + ")";
		}
		res = res + ")";
		String scste = String.format("#x%04x", cste);
		return "("+ opToSmtlib(op1) + " " + res + " " + scste + ")"; 
