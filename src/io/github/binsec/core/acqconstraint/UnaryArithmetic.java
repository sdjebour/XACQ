...
public class UnaryArithmetic extends UnaryConstraint {
	...
	protected boolean check(Bitvec value) {
		Bitvec bvcst = new Bitvec(BigInteger.valueOf(cste), value.size);
		switch (op) {
		case EQ:
		...
		case NQ:
			return !value.equals(bvcst);
		case GT:
			return value.scompareTo(bvcst) > 0;
		case GE:
			return value.scompareTo(bvcst) >= 0;
		case LT:
			return value.scompareTo(bvcst) < 0;
	...
	public String getBwConstraint() {
		int var = variables[0];
		String hexcste = "#x" + String.format("%08x", cste);
		switch (op) {
		case EQ:
			return String.format("(= v%d %s)", var, hexcste);
		case NQ:
			return String.format("(distinct v%d %s)", var, hexcste);
		default:
			assert false : "Operator not handled";
			return null;
		}
