...
public class DerefConstraint extends BinaryConstraint {
	...
	public String getBwConstraint() {
		...
		String val2_ref = "v" + this.cell2.getRef().id;
		String c = String.format("(= %s %s)", val1_value, val2_ref);
		if (!negated)
			return c;
		else
	...
	public String toBinsecConstraint(ACQ_Query q, Map<String, String> regid) {
		...
		String hexaddglob = "#x" + String.format("%0" + cell1.getType().getSize(cell1.arch) / 4 + "x",
				cell2.getRef().addr.unsigned());
		// System.out.println(hexaddglob);
		String c = String.format("(= %s %s)", val1_value, hexaddglob);
		if (!negated)
			return c;
		else
			return String.format("(not %s)", c);
