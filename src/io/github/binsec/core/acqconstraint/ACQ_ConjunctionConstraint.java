...
public class ACQ_ConjunctionConstraint extends ACQ_MetaConstraint {
	...
	public String getBwConstraint() {
		assert constraintSet.size() > 0;
		String res = "(and";
		if (constraintSet.size() == 1) {
			return constraintSet.get_Constraint(0).getBwConstraint();
		}

		for (ACQ_IConstraint c : constraintSet) {
			res += " " + c.getBwConstraint();
		}
		// System.out.println("bw of " + this.toString() + ": " + res + ")");
		return res + ")";
	}
	...
	public String toBinsecConstraint(ACQ_Query q, Map<String, String> regid) {
		...
		String res = "(and";
		if (constraintSet.size() == 1) {
			return constraintSet.get_Constraint(0).toBinsecConstraint(q, regid);
		}

		for (ACQ_IConstraint c : constraintSet) {
			res += " " + c.toBinsecConstraint(q, regid);
		}
		return res + ")";
	}
	...
	public boolean check(ACQ_Query query) {
		for (ACQ_IConstraint c : constraintSet) {
			Bitvec[] value = c.getProjection(query);
	...
	private Bitvec[] ordred_value(ACQ_IConstraint c, Bitvec... value) {

		if (value.length == 0 || !(c instanceof ScalarConstraint))
			return value;

		Bitvec[] result = new Bitvec[value.length];
