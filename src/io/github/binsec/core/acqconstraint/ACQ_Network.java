...
public class ACQ_Network implements Iterable<ACQ_IConstraint> {
	...
	public boolean contains(ACQ_IConstraint cst) {
		...
		for (ACQ_IConstraint cst_ : this.constraints)
			if (cst_.equals(cst)) {
				// System.out.println("THIS :"+cst.getName()+"\nAND \n THIS :"+cst_.getName()+"
				// IS THE SAME\n");
				return true;
			}
	...
	public boolean check(ACQ_Query query) {
		for (ACQ_IConstraint constraint : constraints) {
			if (query.getScope().containsAll(constraint.getScope()) && !constraint.check(query)) {
				return false;
			}
		}
		return true;
	...
	public ACQ_Network getLearnedNetwork(CNF T) {
		for (Clause cl : T.clauses) {
			if (cl.getSize() == 1) {
				Unit u = cl.get(0);
				assert u.negated == false;
				this.add(u.getConstraint(), false);
			}
		}
		return this;
	}

	public ACQ_Network minus(ACQ_Network learned) {
		ACQ_Network res = new ACQ_Network(constraintFactory);
		for (ACQ_IConstraint c : this) {
