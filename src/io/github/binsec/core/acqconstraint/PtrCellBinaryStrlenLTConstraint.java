...
public class PtrCellBinaryStrlenLTConstraint extends BinaryConstraint {
	boolean negated = false;
	ACQ_CellVariable cell1;
	ACQ_CellVariable cell2;
	ACQ_Variable[] vars; // length = 2

	public PtrCellBinaryStrlenLTConstraint(ACQ_CellVariable cell1, ACQ_CellVariable cell2) {
		super("StrlenLT", cell1.getSize().id, cell2.getSize().id); // TODO : WARNING I don't know if it is good
	...
	public PtrCellBinaryStrlenLTConstraint(ACQ_CellVariable cell1, ACQ_CellVariable cell2, boolean negated) {
		this(cell1, cell2);
		if (negated) {
			this.setName("StrlenGE");
	...
	public String toBinsecConstraint(ACQ_Query q, Map<String, String> regid) {
		...
		String c = String.format("(bvult %s %s)", val1_value, val2_value);
		if (!negated)
			return c;
		else
	...
	public ACQ_IConstraint getNegation() {
		return new PtrCellBinaryStrlenLTConstraint(this.cell1, this.cell2, !this.negated);
	}
	...
	public String toSmtlib() {
		String p = "(strlenlt v" + this.cell1.cellid + " v" + this.cell2.cellid + ")";
		if (!negated)
			return p;
		else
			return "(not " + p + ")";
	...
	protected boolean check(Bitvec value1, Bitvec value2) { // Better if prams could be cells
		if (negated) {
			return value2.ucompareTo(value1) <= 0;
		} else {
			return value2.ucompareTo(value1) > 0;
		}
	}
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		PtrCellBinaryStrlenLTConstraint other = (PtrCellBinaryStrlenLTConstraint) obj;
