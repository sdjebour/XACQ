...
public class IntCellBinaryModConstraint extends BinaryConstraint {
	...
	public IntCellBinaryModConstraint(ACQ_CellVariable cell1, ACQ_CellVariable cell2, int res) {
		super("BMod", cell1.getValue().id, cell2.getValue().id); // TODO : WARNING I don't know if it is good
	...
	public IntCellBinaryModConstraint(ACQ_CellVariable cell1, ACQ_CellVariable cell2, int res, boolean negated) {
		this(cell1, cell2, res);
		if (negated) {
			this.setName("BNmod");
	...
	public String getBwConstraint() {
		...
		String hexres = "#x" + String.format("%0" + n + "x", res);
		...
		if (cell1.getType() == CellType.INT || cell1.getType() == CellType.INT8)
			c = String.format("(and (= (bvsrem %s %s) %s) (not (= %s %s)))", val1_value, val2_value, hexres, val2_value,
					hexcst);
		if (cell1.getType() == CellType.UINT)
			c = String.format("(and (= (bvurem %s %s) %s) (not (= %s %s)))", val1_value, val2_value, hexres, val2_value,
					hexcst);
		if (cell1.getType() == CellType.UINT16)
			c = String.format("(and (= (bvurem %s %s) %s) (not (= %s %s)))", val1_value, val2_value, hexres, val2_value,
					hexcst);
		if (cell1.getType() == CellType.UINT8)
			c = String.format("(and (= (bvurem %s %s) %s) (not (= %s %s)))", val1_value, val2_value, hexres, val2_value,
					hexcst);
		if (!negated)
			return c;
	...
	public String toBinsecConstraint(ACQ_Query q, Map<String, String> regid) {
		...
		String c = "";
		if (cell1.getType() == CellType.INT)
			c = String.format("(and (= (bvsrem %s %s) %s) (not (= %s %s)))", val1_value, val2_value, hexres, val2_value,
					hexcst);
		if (cell1.getType() == CellType.INT8)
			c = String.format("(and (= (bvsrem %s %s) %s) (not (= %s %s)))", val1_value, val2_value, hexres, val2_value,
					hexcst);
		if (cell1.getType() == CellType.UINT)
			c = String.format("(and (= (bvurem %s %s) %s) (not (= %s %s)))", val1_value, val2_value, hexres, val2_value,
	...
	public ACQ_IConstraint getNegation() {
		return new IntCellBinaryModConstraint(this.cell1, this.cell2, this.res, !this.negated);
	}
	...
	public String toSmtlib() {
		...
		case INT:
			expr = "(smod " + val1 + " " + val2 + " " + String.format("#x%08x", this.res) + ")";
			break;
		case INT8:
			expr = "(smod " + val1 + " " + val2 + " " + String.format("#x%02x", this.res) + ")";
			break;
		case UINT:
			expr = "(umod " + val1 + " " + val2 + " " + String.format("#x%08x", this.res) + ")";
			break;
		case UINT8:
			expr = "(umod " + val1 + " " + val2 + " " + String.format("#x%04x", this.res) + ")";
			break;
		case UINT16:
			expr = "(umod " + val1 + " " + val2 + " " + String.format("#x%02x", this.res) + ")";
			break;
		default:
	...
	protected boolean check(Bitvec value1, Bitvec value2) { // Better if prams could be cells
		Bitvec bvres = new Bitvec(BigInteger.valueOf(res), value1.size);
		if (value2.equals(Bitvec.zero(value2.size))) {
			return negated;
		}
		boolean res = value1.mod(value2).equals(bvres);
		if (negated)
			return !res;
		else
			return res;
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		IntCellBinaryModConstraint other = (IntCellBinaryModConstraint) obj;
