...
public class Chrono {
	...
	synchronized public void reset(String serieName) {
	    current_chronos.remove(serieName);
	    results.remove(serieName);
	}
	...
	synchronized public void mark(String serieName) {
	    Long startTime = current_chronos.get(serieName);
	    if (startTime != null) {
	        ArrayList<Long> result = results.get(serieName);
	        if (result == null) {
	            result = new ArrayList<Long>();
	        ...
	        result.add(now - startTime); // record elapsed time since start
	    }
	}
