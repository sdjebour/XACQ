...
public class ACQ_CONACQ {
	...
	public ArrayList<ACQ_Query> getAskedQueriesUkn() {
		ArrayList<ACQ_Query> ukn = new ArrayList<>();
		for (ACQ_Query e : asked) {
			if (e.isUnknown()) {
				ukn.add(e);
			}
	...
	public ACQ_Query query_gen(CNF T, ContradictionSet N) throws Exception {
		...
		while (!istimeouted() && q.isEmpty() && !T.isMonomial()) {
			...
			if (model == null) {
				...
				//System.out.println("LE NETWORK "+network.toString());
				...
				if (q.isEmpty()) {
					...
					//System.out.println("THE NETWORK OF THE NEXT QUERY\n"+network.toString());
	...
	protected ACQ_Query irredundantQuery(CNF T) {
		//System.out.println("ON EST A IRREDUNDANT QUERY");
		assert(T.isMonomial());
		...
		for(int i=0;i<learned.size();i++) {
			//System.out.println("LE NETWORK LEARNED : "+learned.getConstraints().get_Constraint(i));
		}
		...
		while(System.currentTimeMillis() - startTime < 100) { // Until 0.1 second timeout
			...
			//System.out.println("THE QUERY :  = "+q.toString2());
	...
	//Warning : strategy always have alias and validity, even when it's not in the bias
	...
	protected boolean preprocess(CNF T, ContradictionSet N) throws Exception {
		...
		for (ACQ_Query membership_query : preprocanswered) {
			...
			if (verbose) System.out.println("[INFO] replay query " + qp.toString(membership_query) + "::" + membership_query.getClassification());
			ConstraintSet kappa = bias_minus.getKappa(membership_query);
			assert kappa.size() > 0;
			if (kappa.size() == 0 && !membership_query.isPositive()) {
				collapse = true;
		...
		while (!stop) {
			...
			if (membership_query.isEmpty()) {
				...
				ConstraintSet kappa = bias_minus.getKappa(membership_query);
				//System.out.println("LE KAPPA : "+kappa.toString2());
				if (verbose) System.out.println("::" + membership_query.getClassification());
				asked.add(membership_query);
				//assert kappa.size() > 0;
				...
				//System.out.println("bias size after query : "+bias_minus.getSize());
	...
	public boolean process() throws Exception {
		...
		while (active && !(collapse || convergence)) {
			...
			if (membership_query.isEmpty()) {
				...
				ConstraintSet kappa = bias_minus.getKappa(membership_query);
				//System.out.println("BIAS NOW:\n"+bias_minus.getConstraints().toString2());
				assert kappa.size() > 0: "non-informative kappa !";
				if (verbose) System.out.println("::" + membership_query.getClassification());
				asked.add(membership_query);
				assert !asked_debug.contains(membership_query.toString()):"non-informative kappa!";
